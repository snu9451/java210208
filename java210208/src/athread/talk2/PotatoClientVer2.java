...
public class PotatoClientVer2 extends JFrame implements ActionListener {
	...
	JButton				jbtn_size		= new JButton("글자크기");
	...
	// JTextArea에는 문자열을 그리는 개념이 아니라 '출력하는 개념'이라서 부분 색상 변경 또는 문자열 그리기 등이 불가능함.
...
//			socket = new Socket("192.168.0.32", 7375);	//희태꺼
			...
			oos.writeObject(Protocol.ROOM_IN + "#" + nickName); // 말한다.
...
//			pct.start();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (jbtn_one == obj) {
			...
			if (afterName == null || afterName.trim().length() < 1) { // 유효성 검사
				JOptionPane.showMessageDialog(this, "값을 입력해주세요.", "INFO", JOptionPane.INFORMATION_MESSAGE);
				return;
			} else {
				try {
					oos.writeObject(300 + "#" + nickName + "#" + afterName);
